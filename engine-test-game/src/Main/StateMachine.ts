class StateMachine extends dessert.DisplayObjectContainer {
    currentState: State;
    // totalEndX: number;
    // totalEndY: number;
    ...
    stand: dessert.MovieClip; //动画站立
    moveR: dessert.MovieClip    // 动画走(向右)
    moveL: dessert.MovieClip    // 动画走(向左)
    fight: dessert.MovieClip    //动画战斗
    timeOnEnterFrame: number = 0;//进入帧时间
    Ratiox: number;
    Ratioy: number;

    constructor(locked: boolean, _player: any,
        stand: dessert.MovieClip, moveR: dessert.MovieClip, moveL: dessert.MovieClip, fight: dessert.MovieClip) {
        super();
        this.locked = 0;
        this.stand = stand;
        this.moveR = moveR;
...
class PlayerMoveState implements State {
    ...
    _currentmove: dessert.MovieClip
