class NPC extends dessert.DisplayObjectContainer implements Observer {
	private _emoji: dessert.Bitmap = new dessert.Bitmap();
	private _accept_mark: dessert.Bitmap = new dessert.Bitmap();
	private _unfinish_mark: dessert.Bitmap = new dessert.Bitmap();
	private _finish_mark: dessert.Bitmap = new dessert.Bitmap();
	private dialoguePanel: DialoguePanel;
	public _id: String;                  //NPC的id
	private _stage: dessert.DisplayObjectContainer;
	private MARK_Y = -120;
	private npc_x: number;
	private npc_y: number;
	//private taskList: Task[] = [];      //
	public constructor(_stage: dessert.DisplayObjectContainer, NpcId: string, emoji: string, accept_mark: string, unfinish_mark: string, finish_mark: string,
		dialoguePanel: DialoguePanel, npc_x: number, npc_y: number) {
		super();
		this._stage = _stage;
		this._emoji.imageResource = dessert.res.get(emoji);
		this._accept_mark.imageResource = dessert.res.get(accept_mark);
		this._unfinish_mark.imageResource = dessert.res.get(unfinish_mark);
		this._finish_mark.imageResource = dessert.res.get(finish_mark);
		this._id = NpcId;
